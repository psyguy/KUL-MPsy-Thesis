...
names.firstgen <- list.files(path = "./data/", pattern = "eps-") %>% sort()
nn <- gsub(".*2k_","",names.firstgen)
nnn <- gsub("_.*","",nn)


coefs.all <- coefs.all %>% filter(Owner %in% nnn)
names.list <- coefs.all$Owner %>% unique() %>% as.character()
i <- 1
t3 <- Sys.time()
