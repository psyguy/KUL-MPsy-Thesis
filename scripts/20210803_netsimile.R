...
load("./data/last-snapshot-conn-activity_20191010_1926.RData")
...
# Extracting anatomical and functional connectivities ---------------------

### Renaming based on new family names/codes
...
names(l.extracted) <- gsub(".*_","",names(l.extracted))

for(b in 1:50){
  name <- names(l.extracted)[b]
  names(l.extracted)[b] <- snp.micro[snp.micro$Owner==name,"fn"]
}
...
signatures.connectivities <- features.connectivities %>%
  ldply(featAggr) %>%
  arrange(.id)
signatures.activities <- features.activities %>%
  ldply(featAggr) %>%
  arrange(.id)

## saving NetSimile distance matrix
...
save_vars(c("s.con", "s.act"), prefix = "NetSimile-distances")
